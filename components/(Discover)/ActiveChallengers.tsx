...
const ActiveChallengers = ({
  ...
  setViewers: (viewers: any[]) => void; // Add this
}) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // SEND VIEWER DATA TO PARENT/HOMEPAGE
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 p-2">
      {allPlayers.map((player) => {
        ...
        return (
          <div
            key={playerId}
            className="flex flex-col gap-5 group w-full max-w-[420px] mx-auto"
          >
            <div
              onClick={() => {
                console.log(`🖱️ [UI] Opening profile: ${player.name}`);
                if (!user) {
            ...
            >
              ...
              >
                <StatusBadge
                  status={player.statusBadge || "DONKEY"}
                  size="sm"
                />
              </div>
              ...
              {voiceUrl && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    console.log("🎵 [AUDIO] Playing intro...");
            ...
            <button
              onMouseEnter={() => router.prefetch(`/play/record/${playerId}`)} // PRE-LOADS the page when mouse hovers!
              onClick={() => {
                if (!user) {
      ...
      <ChallengeModal
        ...
        onConfirm={(name: string) => {
          ...
          router.push(
            `/play/record/${modal.id}?donkeyName=${encodeURIComponent(name)}`,
          );
        }}
